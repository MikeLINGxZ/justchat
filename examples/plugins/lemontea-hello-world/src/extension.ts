...
// ---- 类型定义（与 extension-host/src/types.ts 对齐） ----
...
export async function activate(api: LemonTeaAPI) {
  ...
  api.tools.register({
    ...
    execute: async (params: { language?: string }) => {
  ...
  api.tools.register({
    ...
    execute: async (params: { sides?: number; count?: number }) => {
...
- 你的回复要简洁有趣，不要太长`,
  ...
  // ======== 4. 注册 Hook：对话统计 ========
  const beforeHook = api.hooks.onBeforeChat(async (ctx: any) => {
    // 统计用户发起的对话次数
  ...
  const afterHook = api.hooks.onAfterChat(async (ctx: any) => {
    // 记录最后一次对话时间
