...
func NewWriteMemoryTool(storage *storage.Storage) func(ctx context.Context, in *WriteMemoryToolRequest) (out *WriteMemoryToolResponse, err error) {
	return func(ctx context.Context, in *WriteMemoryToolRequest) (*WriteMemoryToolResponse, error) {
		// 初始化响应
		...
		// 如果 WriterMemory 返回了 ID，也可以在这里补充（需要修改 WriterMemory 接口）
		// 例如：response.MemoryID = memory.ID
...
type ReadMemoryToolRequest struct {
	...
	StartAt *time.Time `json:"start_at,omitempty" jsonschema:"title=起始日期;description=只返回此日期之后（含）发生的记忆，格式 YYYY-MM-DD；required=false"`
	EndAt   *time.Time `json:"end_at,omitempty" jsonschema:"title=结束日期;description=只返回此日期之前（含）发生的记忆，格式 YYYY-MM-DD；required=false"`
}
...
func NewReadMemoryTool(storage *storage.Storage) func(ctx context.Context, in *ReadMemoryToolRequest) (out *ReadMemoryToolResponse, err error) {
	return func(ctx context.Context, in *ReadMemoryToolRequest) (out *ReadMemoryToolResponse, err error) {
		// 初始化响应对象
		...
		// 调用 Storage 层查询数据
		memories, err := storage.ReadMemory(ctx, in.Keyword, in.StartAt, in.EndAt)
		if err != nil {
			response.Message = "查询记忆失败: " + err.Error()
			return response, nil // 返回 response 而不是错误，便于 AI 理解
		}
